...
** smaller value.)
获取pc所在的某一段源代码的第一行行号，
因为absline是以MAXIWTHABS作为间隔进行采样的，
所以在外面的调用又可能在某一段中是找不到pc所在的行的，
所以这个方法是用于查找pc所在的起始absline，
可以这样理解lineinfo和abslineinfo的关系：
function {
absline1 --采样
  line1
  line2
...
给状态机设置勾子获取取消勾子，设置了勾子后会给当前的所有调用增加陷阱
*/
...
/*
获取指定层级的CallInfo（即是一个调用栈），层级表示如下：
callinfo 0 --this is the target we want
--callinfo level-...
----...
-----callinfo --level-1
*/
...
/*
获取第n个变参的栈位置，
按照这里获取的方法，参数应该是从右往左顺序入栈，而且变参是单独一块区域，
就是说和其他有名参数的布局方式不一样，大概可以理解为这样：
stack func
vargs .
vargs .
vargs .
...
所以获取第n个变参的n是负数，比如获取第二个n应该就是-2
*/
static const char *findvararg(CallInfo *ci, int n, StkId *pos)
{
  if (clLvalue(s2v(ci->func))->p->is_vararg)
  {
    int nextra = ci->u.l.nextraargs; //获取变参的参数个数
    if (n >= -nextra)
    { /* 'n' is negative */
...
/*
获取调用栈中的第n个局部变量，如果获取到：返回变量名，并且告知变量在栈中的哪个位置pos???
*/
...
/*
获取当前方法调用中，第n个局部变量的名称，
这里会根据传入参数ar来进行不一样的处理，为什么？？？
*/
...
static int changedline(const Proto *p, int oldpc, int newpc)
{
  ...
  {                /* not too far apart? */
    ...
    for (;;)
    {
      // lineinfo是指令所在的行号，是一个偏移量（也就是当前指令相对于上一条指令的源代码行号便宜）
