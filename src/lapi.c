...
获取指定索引的值。这里根据idx的值分几种情况：
1. >0: 这个索引是当前调用call的栈中的索引
2. > -1001000(LUA_REGISTRYINDEX): 在L的栈中来取值
3. == -1001000(LUA_REGISTRYINDEX): 取l_registry（寄存器）
4. < -1001000(LUA_REGISTRYINDEX): 取当前调用（只能是c闭包）的upvalue，如果不是c闭包，返回nil
*/
...
/**
 * @brief 检查lua状态机的栈空间是否足够>=n，如果不够，扩展栈空间
 *
 * @param L
 * @param n
 * @return LUA_API
...
/**
 * @brief 把from中的栈数据拷贝n个元素到to中
 * 1. 检查from中是否有足够的元素
 * 2. 检查from和to是否同属于一个global_state
 * 3. 检查to的栈空间是否足够（所以要求提前分配好栈空间）
 * 4. 逐个元素进行拷贝
 *
 * @param from
 * @param to
...
这是一个数组旋转算法，就是把指定索引之前的元素按照顺序挪到后面。
*/
...
/**
 * @brief 把fromidx位置的值拷贝到toidx，如果toidx是一个upvalue，对from添加内存屏障
 *
 * @param L
 * @param fromidx
 * @param toidx
...
/**
 * @brief 把整数或者浮点数转换为number，如果idx位置对应的是字符串，把字符串也转换为对应的数值
 * 返回转换的数值，并且把是否成功转换标记记录到pisnum中
 *
 * @param L
 * @param idx
 * @param pisnum
...
/**
 * @brief 把整数或者浮点数转换为integer，如果idx位置对应的是字符串，把字符串也转换为对应的integer
 * 返回转换的数值，并且把是否成功转换标记记录到pisnum中
 *
 * @param L
 * @param idx
...
/**
 * @brief 把idx对应的位置转换成bool，如果对应值是false或者nil也会转换为false，其他都是true
 *
 * @param L
 * @param idx
 * @return LUA_API
...
/**
 * @brief 把idx对应位置转换到string,如果转换失败，设置len=0，并返回NULL，可能会导致GC
 *
 * @param L
 * @param idx
 * @param len
...
/**
 * @brief 获取string, userdata和table的长度
 * 对于table来说，如果存在数组部分，则获取数组部分的长度，否则获取hash部分的size
 *
 * @param L
 * @param idx
 * @return LUA_API
...
/**
 * @brief 从L的global table中获取指定key(name)对应的value
 *
 * @param L
 * @param name
 * @return LUA_API
...
/**
 * @brief 将栈顶的值作为key从idx对应的table中获取value
 * 这里是把value放到slot中，然后把slot的值取替换栈顶的值，如果拿不到，取操作元表获取
 *
 * @param L
 * @param idx
 * @return LUA_API
...
/**
 * @brief 获取idx对应的table中，key为整数n的value
 * 获取的顺序：
 *     1）判断数组部分，如果可以，从数组中拿
...
/**
 * @brief 顾名思义，rawget, 将idx对应的table，并将栈顶的值作为key
 * 取table中拿值，如果没有呢，那也结束了，不会去动元表
 *
 * @param L
 * @param idx
 * @return LUA_API
...
/**
 * @brief 和lua_rawget一样，只是key是直接传入的整数值
 *
 * @param L
 * @param idx
 * @param n
...
/**
 * @brief 和lua_rawget一样，只是key为void* p
 *
 * @param L
 * @param idx
 * @param p
...
/**
 * @brief 创建一个指定了数组部分和hash部分长度的table
 *
 * @param L
 * @param narray
 * @param nrec
...
/**
 * @brief 设置idx对应的table的key-value(不操作元表)
 * key: top-2
 * value: top-1
 *
 * @param L
...
/**
 * @brief 把p(userdata)作为key设置idx对应table的value(不操作元表)
 *
 * @param L
 * @param idx
 * @param p
...
/**
 * @brief 把整数n(integer)作为key设置idx对应table的value(不操作元表)
 *
 * @param L
 * @param idx
 * @param n
...
/**
 * @brief 把栈顶的值作为metatable设置到objindex对应的元素中
 *
 * @param L
 * @param objindex
 * @return LUA_API
...
/*
加载源代码并解析成字节码，并设置加载字节码的upvalue[0]为全局寄存器
*/
