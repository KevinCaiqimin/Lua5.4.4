...
把一个luavalue转换为number,如果luavalue是一个string,也根据规则强制转换为number
*/
...
获取key对应value，当外层在table中拿值拿不到的时候，会走到这里，走到这里的时候，slot一定是empty或者NULL,
然后从t的metatable的__index中拿对应key的值，当然如果还是拿不到，这个过程是递归的
*/
...
设置table的key-val，设置流程：
优先使用__newindex元表操作，如果__newindex是一个function，那就直接调用执行，并返回，
如果__newindex不是一个方法（那就必须是一个table)，如果这个元表中存在这个key对应的坑位，
直接设置了之后返回，否则继续递归元表的元表
*/
